#!/usr/bin/env python3
"""
make_video_anim.py — CSV → MP4/GIF replay for longitudinal_1d_sim.

Reads the newest CSV (t,x,v,d,brake_cmd,u) from ~/ros2_ws/logs and renders:
  • Left: car vs wall + brake light (red when brake_cmd==1) + live HUD
  • Right: v(t) and d(t) plots with a moving time cursor
Saves MP4 to ~/ros2_ws/results/videos/ (falls back to GIF if ffmpeg is missing).
- This is a viewer only; it does not read ROS params. Displayed rt/margin are labeled ASSUMED_*.
"""
import csv
import glob
...
with open(csv_path, 'r') as f:
    ...
    for row in reader:
        ...
        # older logs may not have 'u'
        u.append(float(row.get('u', '0.0')))

if not t:
    raise SystemExit(f'Empty CSV: {csv_path}')

# ---------- derived constants (display only) ----------
x_obs = x[0] + d[0]                 # obstacle position at start
car_len, car_h = 4.0, 1.6           # purely visual
dt = (t[1] - t[0]) if len(t) > 1 else 0.05
ASSUMED_RT = 1.0
ASSUMED_MARGIN = 0.10
# Estimate decel magnitude from |u| (if present) just for display:
decel_est = max(1e-6, max(abs(ui) for ui in u) if u else 0.0)

# ---------- figure/text layout ----------
fig = plt.figure(figsize=(10, 5), constrained_layout=True)
fig.suptitle('ROS 2 closed-loop replay: sim ↔ brake_decider', fontsize=12, fontweight='bold')
fig.text(0.02, 0.10,
         "Topics: /ego_speed   /obstacle_distance   /brake_cmd\n"
         f"Params (display): rt={ASSUMED_RT:.2f}s  margin={ASSUMED_MARGIN:.2f}  decel≈{decel_est:.1f} m/s^2",
         fontsize=8)
fig.text(0.02, 0.20,
         'Decision: BRAKE if d_stop ≥ d_avail\n'
         ' Where: d_stop = v*rt + v^2/(2a)\n'
         '        d_avail = d*(1 - m)',
         fontsize=10)
...
# rear brake light (small square near the rear edge)
side   = car_h * 0.8
margin = -1.0
light_y = (car_h - side) / 2.0
light = plt.Rectangle((x[0] - car_len + margin, light_y),
                      side, side,
                      fill=True, facecolor='red',
                      zorder=6, visible=bool(b[0]))
ax_world.add_patch(car)
ax_world.add_patch(light)
...
ax_d.set_title('Distance d(t)')
ax_d.plot(t, d, linestyle='--')
ax_d.set_xlabel('t [s]')
ax_d.set_ylabel('d [m]')
...
# ---------- animation ----------
def update(i: int):
    # move car + brake light visibility
...
try:
    fps = int(round(1.0 / dt)) if dt > 0 else 20
    fps = max(5, min(60, fps))  # clamp for portability
